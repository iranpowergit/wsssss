...
local function delmsg (MaTaDoR,samtaylor)
    msgs = MaTaDoR.msgs 
    for k,v in pairs(samtaylor.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(samtaylor.messages_[0].chat_id_,{[0] = samtaylor.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(samtaylor.messages_[0].chat_id_, samtaylor.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
...
text = string.gsub(text, "no", "`Unlock🔓`")
 ...
 text =  string.gsub(text, "no", "قفل باز🔓")
 end
...
tdcli.changeChatTitle(chat, gp_name, dl_cb, nil)
 ...
 if not data[tostring(chat)]['about'] then
  ...
  if matches[1] == 'unfilter' and is_mod(msg) or matches[1] == 'Unfilter' and is_mod(msg) or matches[1] == 'حذف فیلتر' and is_mod(msg) then
...
======ManaGer====== 
This Help List Only For вℓα¢к/Owners! 
...
🔸️🔹️🔸️🔹️🔸️🔹️🔸️🔹️🔸️🔹️ 
======SuDo and Admin====== 
Tr fa This help is only for sudoers/bot admins. 
...
>راهنمای حذفی< 
 ...
 ======پایان دستورات====== 
          🔸🔹🔸🔹🔸🔹🔸🔹🔸🔹 
======دستورات سودو و ادمین====== 
این دستور فقط برای مالک ربات همان سودو است 
 
⬅️راهنمای سودو
> دستورات‌صاحب‌ربات < 

======پایان دست سودو و ادمین===== 
          🔸🔹🔸🔹🔸🔹🔸🔹🔸🔹
======اعضا====== 
⬅️راهنمای فان
> دستورات‌سرگرم‌کننده < 
...
if matches[1] == "helpsudo" and is_mod(msg) or  matches[1] == "Helpsudo" and is_mod(msg) or  matches[1] == "راهنمای سودو" and is_mod(msg) then
text = [[
🔖*нєℓρ вℓα¢к ™*
...
===================
*Setlang* `[fa - en]`
♦️Language Set the robot to respond to commands

*Silent* `[username , id , reply]`
➡️Muting person each person give a message that is deleted
...
♦️Block the person and not to allow re-entry

*Unban* `[username , id , reply]`
➡️One block off of state 

*Delall* `[username , id , reply]`
♦️Delete all messages that the person is in the group

*Filter* `[text]`
➡️Filter word and delete it

*Unfilter* `[text]`
♦️Delete the word to the filter list

*Welcome* `[enable-disable]`
➡️On and off the welcoming

*Promote* `[username , id , reply]`
➡️Set one as vice Robot Group

*Demote* `[username , id , reply]`
➡️One of the assistant robot demoted to Group

*Settings*
➡️Get a list of deputy group

*Modelist*
➡️Get a list of deputy group

*Ownerlist*
♦️Get a list of group administrators 

*Silentlist*
➡️Get the list are silent

*Filterlist*
♦️Get a list of words filter

*Setflood* `[1-50]`
➡️ repeated message

*Set* `[rules , name , link , about]`
♦️Set to the [rules, names, links, information] for group

*Setwelcome* `[text]`
➡️Set welcome to Group
...
*Pin* `[reply]`
♦️PIN message to the group

*Unpin* `[reply]`
➡️Remove the message from the pin
...
*Link*
➡️Download Link Group

 *🌐 این دستوارت مختص زبان خاصی نیست !*
_To Change The LanGuage_ `[Setlang]` `[en , fa]`
...
⬅️ساکت کردن فرد (هر پیامی که فرد بدهد پاک میشود)

*رفع سکوت* `[username , id , reply]`
...
⬅️خارج شدن فرد از حالت مسدود 

*حذف پیام* `[username , id , reply]`
♦️پاک کردن تمام پیام هایی که فرد داده است در گروه

*فیلتر* `[text]`
⬅️فیلتر کلمه مورد نظر و پاک شدن ان

*حذف فیلتر* `[text]`
♦️حذف کلمه مورد نظر از لیست فیلتر

*ولکام* `[فعال-غیر فعال]`
⬅️فعال و غیر فعال کردن خوش آمد گوی

*ترفیع* `[username , id , reply]`
⬅️تنظیم فرد به عنوان معاون ربات در گروه

*تنزل* `[username , id , reply]`
♦️تنزل مقام فرد از معاون ربات برای گروه

*تنظیمات*
♦️دریافت لیست تنظیمات

*لیست ناظم*
⬅️دریافت لیست معاون های گروه

*لیست مالکان*
♦️دریافت لیست مدیران گروه 

*لیست ساکت*
⬅️دریافت لیست ساکت ها
...
if not lang then
text = [[
💠*EngliSh CoMmAnDs FUN :*
------------------------------
*Azan* `[name city]`
🔱Azan town to get information
----------------------------
*Info*
♦️Get the information you
----------------------------
*Me*
🔱Get your place in the Robot
----------------------------
*Ping*
♦️Of the robot online
----------------------------
*Time*
🔱Get time and date today
----------------------------
*Write* `[text]`
♦️Write the word with 100 different fonts
----------------------------
*Nerkh*
🔱Get the robot for the purchase price
----------------------------
*Weather* `[city]`
♦️Get to climatic City
----------------------------
*Calc* `[number]`
🔱calculator
----------------------------
*Tr* `[lang] [word]`
♦️Farsi to English text translation vice versa
_Example:_
_Tr en سلام_
----------------------------
*Short* `[link]`
...
if matches[1] == "helpdel" and is_mod(msg) or matches[1] == "Helpdel" and is_mod(msg) or matches[1] == "راهنماحذفی" and is_mod(msg) then
if not lang then
text = [[☑️help delete Black

♦️Wipe with a message Ryplay only commands :

🔱 del [reply]
----------------------------
♦️Clear your personal message commands :

🔱 mydel
----------------------------
♦️Clear the number of message commands :

🔱 del [2-1000]
----------------------------
♦️Remove as much as possible commands :

🔱[!#/] rmsg all
----------------------------
♦️According commands Persian and English language
----------------------------
👤 *ρσωєяɗ Ɓу :* @GODILOVEYOUME2
🎭 *ƇнαηηєƖ :* @TeamBlackbot
]]
elseif lang then
text = [[☑️راهنمای حذفی بلک

♦️پاک کردن با ریپلای فقط یه پیام دستورات :

🔱حذف[پاسخ]
----------------------------
♦️پاک کردن پیام شخصی خود دستورات :

🔱پاکسازی پیام های من
----------------------------
♦️پاک کردن تعداد پیام دستورات :

🔱حذف [2-1000]
----------------------------
♦️پاک کردن تا جای‌ ممکن دستورات :

🔱پاکسازی همه
----------------------------
♦️توجه دستورات دارای زبان فارسی و انگلیسی است
----------------------------
👤 *برنامه نویس Ɓу :* @GODILOVEYOUME2
🎭 *کانال :* @TeamBlackbot
]]
...
برای خرید دستور پرداخت را ارسال کنید💳 .
(پرداخت)
*________________*
...
if matches[1] == "bot" or matches[1] == "Bot" or matches[1] == "ربات" then
text = [[
انلاین هستم عزیز 
]]
return text
...
if not lang then
text = [[
̶B̶E̶L̶A̶K̶ ̶M̶υ̶т̶є̶т̶ι̶м̶є̶ ̶Ƈ̶σ̶м̶м̶α̶η̶ɗ̶ѕ:
...
🔹Mute groups
=============
*°•➡️ Mute* (hour) (minute)  (seconds)
🔸Mute group at this time 
=============
*°•➡️ Mutehours* (number)
🔹Mute group at this time 
=============
*°•➡️ Muteminutes* (number)
🔸Mute group at this time 
=============
*°•➡️ Muteseconds* (number)
🔹Mute group at this time 
=============
*°•➡️ Unmute all*
